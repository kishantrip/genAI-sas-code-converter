...
model = ChatOpenAI(model="gpt-3.5-turbo", temperature=0.0, request_timeout=180,
                   max_retries=0, cache=True)
model2 = ChatOpenAI(model="gpt-3.5-turbo", temperature=0.0, request_timeout=300,
                    max_retries=0, cache=True)

# model = OpenAI(model="babbage", temperature=0.0, request_timeout=180, max_retries=0)
max_chunk = 2500
...
def chunk_checking_and_hitting(prompt, i, max_chunk):
    manual_conversion_text = '''\n\n\n########Manual conversion needed since context length is more than what model 
    allows###########\n {}
    ##########manual block ends#############\n\n\n'''
    if (len(prompts[i * 2]) <= max_chunk) & (len((i * 2) + 2) <= max_chunk):
        pyspark_code = chain.batch(prompt[i * 2: (i * 2) + 2])
        converted_code = os.linesep.join([str(elem) for elem in pyspark_code])
    elif len(prompts[i * 2]) > max_chunk:
        pyspark_code = chain.batch(prompt[(i * 2) + 2])
        pyspark_code = [manual_conversion_text.format(prompts[i * 2])] + pyspark_code
        converted_code = os.linesep.join([str(elem) for elem in pyspark_code])
    elif len(prompts[(i * 2) + 2]) > max_chunk:
        pyspark_code = chain.batch(prompt[i * 2])
        pyspark_code = pyspark_code + [manual_conversion_text.format((i * 2) + 2)]
        converted_code = os.linesep.join([str(elem) for elem in pyspark_code])
    else:
        pyspark_code = [manual_conversion_text.format(i * 2)] + [manual_conversion_text.format((i * 2) + 2)]
        converted_code = os.linesep.join([str(elem) for elem in pyspark_code])
...
for i in range(range_loop):
    ...
    try:
        ...
        print(e)
        time.sleep(60)
        converted_code = chunk_checking_and_hitting(prompts, i, max_chunk)
        with open("converted_code.txt", "a", encoding='utf-8') as f:
            f.write(converted_code)
        with open("converted_code_sas.txt", "a", encoding='utf-8') as f:
            converted_code_sas = os.linesep.join(
